...
char BorS; 	//indicator of system type: HCB or spin. B or Spin

// 1D single-fermion Hamiltonian (size: Nsite x Nsite) used to store eigenstates
double * Ham0;	// initial hamiltonian, with trap centered
double * HamT; 	// after the onset of trap displacement
...
double ratioA, ratioB; // For super-lattice use
...
// State Init - constructs an identity matrix of size Nsize x Nsize.
...
void HamGen(double * H, int ** F, double disp, char Flag)
{
    ...
    //Diagonal elements due to on-site potential
    A=ratioA/ratioB;
