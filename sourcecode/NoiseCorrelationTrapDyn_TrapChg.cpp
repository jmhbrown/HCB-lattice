...
void StateInit(int **);
void HamGen(double *, int **, double, double, char);
...
double T, U, V0, V1; //energy coeffecients: hopping, lattice potential, trap potential
...
int main(int argc, char ** argv)
{
    ...
    if(irank==0)    //data input to Master
    {
        ...
        ip>>Nfermion>>Nsite>>T>>V0>>V1>>PeriodicFlag>>Nbzone>>BorS>>K;
        ...
        MPI::COMM_WORLD.Bcast(&V0, 1, MPI::DOUBLE, 0);
        MPI::COMM_WORLD.Bcast(&V1, 1, MPI::DOUBLE, 0);
    ...
    HamGen(Ham0, F, 0.0, V0, PeriodicFlag);
	HamGen(HamT, F, xdisp, V1, PeriodicFlag);
...
void HamGen(double * H, int ** F, double disp, double Vtrap, char Flag)
{
    int * temp;
    ...
    for(i=0; i<Nsite; i++)
        for(int k=0; k<Nsite; k++)
            H[i*Nsite+i]+=2*cos(2.0*Pi*A*(k+1))*F[i][k]*U+pow(double(k)-xc, 2.0)*F[i][k]*Vtrap;
    else
    for(i=0; i<Nsite; i++)
        for(int k=0; k<Nsite; k++)
            H[i*Nsite+i]+=pow(double(k)-xc, 2.0)*F[i][k]*Vtrap;        
...
void EnergChk(double * KEnerg1, double * KEnerg2, double * GSEnerg2, cdouble * rho, cdouble * Nk)
{
	...
	for(int i=0, j1, j2; i<Nsite; i++)
	{
        ...
        // 
		pot1+=U*2.0*cos(2.0*Pi*i*ratioA/ratioB)*rho[i*Nsite+i]+V1*pow(double(i)-xc, 2.0)*rho[i*Nsite+i];
	}
...
void output(cdouble * rho, cdouble * Nk, cdouble * Noise)
{
    ...
    densfile<<"Output/DensTrap_V"<<V0<<"V"<<V1<<"L"<<Nsite<<"N"<<Nfermion<<"time"<<xtime;
    momfile<<"Output/MomTrap_V"<<V0<<"V"<<V1<<"L"<<Nsite<<"N"<<Nfermion<<"time"<<xtime;
    noisefile<<"Output/NoiseTrap_V"<<V0<<"V"<<V1<<"L"<<Nsite<<"N"<<Nfermion<<"time"<<xtime;
...
void ChkOutput(double ke1, double ke2, double e1, double e2)
{
	...
	cfile<<"CheckFile/ChkTrap_V"<<V0<<"V"<<V1<<"L"<<Nsite<<"N"<<Nfermion<<"X"<<xdisp;
	string s=cfile.str();
	char name[100];
